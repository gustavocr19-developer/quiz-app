...
function armazenarNome(): void {
  const nomeUsuarioInput = document.getElementById('input-name') as HTMLInputElement;
  ...
  if (nomeUsuarioInput && nomeUsuarioInput.value.trim() !== '') {
    ...
    const mensagem = criarMensagem('Por favor, digite seu nome antes de começar.');
    document.body.appendChild(mensagem);
